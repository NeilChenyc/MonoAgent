...
<dict>
    ...
    <string>${projectRoot}/logs/monoagent.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/monoagent.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.monoagent');
  } catch {
...
/**
 * Kill any orphaned monoagent node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned monoagent processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/monoagent.service';
    ...
    unitPath = path.join(unitDir, 'monoagent.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=MonoAgent Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/monoagent.log
StandardError=append:${projectRoot}/logs/monoagent.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned monoagent processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable monoagent`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start monoagent`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active monoagent`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-monoagent.sh');
  const pidFile = path.join(projectRoot, 'monoagent.pid');

  const lines = [
    '#!/bin/bash',
    '# start-monoagent.sh — Start MonoAgent without systemd',
    ...
    '    echo "Stopping existing MonoAgent (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting MonoAgent..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/monoagent.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/monoagent.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "MonoAgent started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/monoagent.log"`,
  ];

...
This skill adds Telegram support to MonoAgent, then walks through interactive setup.
...
```bash
git remote add telegram https://github.com/qwibitai/monoagent-telegram.git
```
...
- Check bot is started: `tail -f logs/monoagent.log`
...
6. Rebuild: `npm run build && launchctl kickstart -k gui/$(id -u)/com.monoagent` (macOS) or `npm run build && systemctl --user restart monoagent` (Linux)

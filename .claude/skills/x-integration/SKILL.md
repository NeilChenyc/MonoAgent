---
name: x-integration
description: X (Twitter) integration for MonoAgent. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** MonoAgent v1.0.0. Directory structure may change in future versions.
...
1. **MonoAgent is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep monoagent (macOS) or systemctl --user status monoagent (Linux)
```
...
| `logs/monoagent.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into MonoAgent, make the following modifications:
...
**Verify success:**
```bash
launchctl list | grep monoagent  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status monoagent
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/monoagent.log | tail -20

# Script errors
grep -i "error\|failed" logs/monoagent.log | tail -20
```
...
docker run monoagent-agent ls -la /app/src/skills/
```

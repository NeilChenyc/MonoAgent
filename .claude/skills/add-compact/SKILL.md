...
> **Note:** `upstream` is the remote pointing to `qwibitai/monoagent`. If using a different remote name, substitute accordingly.
...
1. Start MonoAgent in dev mode: `npm run dev`
...
```bash
git clone <your-fork> /tmp/monoagent-test
cd /tmp/monoagent-test
...
- **No config file.** MonoAgent's philosophy is customization through code changes, not configuration sprawl.

...
Adds Parallel AI MCP integration to MonoAgent for advanced web research capabilities.
...
- **Non-blocking Design** - Uses MonoAgent scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. MonoAgent already set up and running
...
Add Parallel AI MCP servers after the monoagent server:
```typescript
const mcpServers: Record<string, any> = {
  monoagent: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__monoagent__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__monoagent__send_message
   4. Use mcp__monoagent__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo monoagent-agent:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep monoagent  # macOS
...
- Check task runs: `tail -f logs/monoagent.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.monoagent` (macOS) or `systemctl --user restart monoagent` (Linux)

...
│  │    • mcp__monoagent__* (scheduler tools via IPC)                │    │
│  │                                                                │    │
│  └──────────────────────────────────────────────────────────────┘    │
│                                                                       │
└───────────────────────────────────────────────────────────────────────┘
...
│   ├── monoagent.log               # Host stdout
│   └── monoagent.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.monoagent.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'monoagent-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your monoagent installation
...
MonoAgent uses a hierarchical memory system based on CLAUDE.md files.
...
7. Router invokes Claude Agent SDK:
   ...
   └── mcpServers: monoagent (scheduler)
   │
   ▼
...
MonoAgent has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__monoagent__schedule_task]
        {
...
### MonoAgent MCP (built-in)

The `monoagent` MCP server is created dynamically per agent call with the current group's context.
...
MonoAgent runs as a single macOS launchd service.

### Startup Sequence

When MonoAgent starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned MonoAgent containers from previous runs
...
### Service: com.monoagent

**launchd/com.monoagent.plist:**
```xml
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/monoagent.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/monoagent.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.monoagent.plist ~/Library/LaunchAgents/

# Start service
launchctl load ~/Library/LaunchAgents/com.monoagent.plist

# Stop service
launchctl unload ~/Library/LaunchAgents/com.monoagent.plist

# Check status
launchctl list | grep monoagent

# View logs
tail -f logs/monoagent.log
```
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep monoagent` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; MonoAgent auto-starts container runtime but may fail |
...
- `logs/monoagent.log` - stdout
- `logs/monoagent.error.log` - stderr
